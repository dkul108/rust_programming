    ...
    match subcommand.as_str() {
        ...
        "crop" => {
            if args.len() != 6 {
                ...
                let x = args.remove(0).parse().unwrap();
                let y = args.remove(0).parse().unwrap();
                let width = args.remove(0).parse().unwrap();
                let height = args.remove(0).parse().unwrap();
...
fn brighten(infile: String, outfile: String) {
    ...
    img2.save(outfile).expect("crash save outfile");
...
fn generate(outfile: String) {
    ...
    let imgbuf = image::ImageBuffer::new(800, 800);
    for (x, y, pixel)  in imgbuf.enumerate_pixels_mut() {
        *pixel = image::Rgb([0.1, 0.5, 1.0]);
    }
    ...
    // See blur() for an example of how to save the
